...
function openSideInputModal(){if(!closed||pts.length<3)return void showToast("Спочатку замкніть контур");let h="";for(let i=0;i<pts.length;i++){let j=(i+1)%pts.length,isArc="arc"===wallTypes[i];h+=`<div style="margin-bottom:14px;border:1.5px solid #e2e8f0;border-radius:12px;padding:10px"><div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><label style="flex:1;font-weight:700;margin:0">${N(i)+N(j)}</label><button type="button" onclick="toggleSideArcType(${i})" id="sideArcBtn_${i}" style="padding:6px 10px;border-radius:9px;font-size:11px;font-weight:800;background:${isArc?"linear-gradient(135deg,#7c3aed,#a78bfa)":"#f1f5f9"};color:${isArc?"#fff":"#475569"};box-shadow:none">〜 ${isArc?"Дуга":"Пряма"}</button></div><input type="number" inputmode="decimal" class="bulk-side-input"\ndata-i="${i}" value="${lengths[i]||""}" placeholder="Довжина хорди ${N(i)}${N(j)}, см"\nstyle="width:100%;padding:10px;border:1px solid #dbe4f0;border-radius:10px"><div id="sideArcEditor_${i}" style="${isArc?"":"display:none"}">${renderArcPointsEditor(i)}</div></div>`}document.getElementById("sideInputsContainer").innerHTML=h,document.getElementById("sideInputModal").classList.add("open"),setTimeout(()=>{const first=document.querySelector(".bulk-side-input");first&&(currentSideIndex=0,first.focus())},100),document.querySelectorAll(".bulk-side-input").forEach((el,index,arr)=>{function goNext(){index<arr.length-1&&(arr[index+1].focus(),arr[index+1].select())}el.addEventListener("keydown",e=>{"Enter"===e.key&&(e.preventDefault(),goNext())}),el.addEventListener("change",()=>{setTimeout(goNext,50)}),el.addEventListener("focus",()=>{currentSideIndex=index,_wallSideFlash=+el.dataset.i,draw()}),el.addEventListener("blur",()=>{setTimeout(()=>{document.activeElement&&document.activeElement.classList&&document.activeElement.classList.contains("bulk-side-input")||(_wallSideFlash=-1,draw())},30)})})}
/* v11: optional bulk wall-dimension editor. Original one-by-one editor remains untouched. */

/* v13 — real "all dimensions" table. Original one-wall editor is untouched. */
function openAllSideInputsModalV11(){
  if(!closed||pts.length<3)return void showToast("Спочатку замкніть контур");
  let h='<div class="aceil-all-table-v13">';
  for(let i=0;i<pts.length;i++){
    const j=(i+1)%pts.length,name=N(i)+N(j),isArc=wallTypes[i]==="arc";
    h+=`<div class="aceil-all-row-v13" data-row="${i}">
      <button type="button" class="aceil-all-name-v13">${name}</button>
      <div class="aceil-all-input-v13"><input type="number" inputmode="decimal" enterkeyhint="${i===pts.length-1?"done":"next"}" data-i="${i}" value="${lengths[i]||""}" placeholder="0"><span>см</span></div>
      <button type="button" class="aceil-all-arc-v13 ${isArc?"is-arc":""}" title="Пряма / дуга">〜</button>
      <span class="aceil-all-ok-v13">${lengths[i]>0?"✓":""}</span>
    </div>`;
  }
  h+='</div>';
  const box=document.getElementById("allSideInputsContainerV11");
  box.innerHTML=h;
  document.body.classList.add("aceil-all-mode-v13");
  document.getElementById("allSideInputsModalV11").classList.add("open");
  const fields=[...box.querySelectorAll("input[data-i]")];
  let active=0;
  const activate=(index,selectIt=false)=>{
    if(!fields.length)return;
    active=Math.max(0,Math.min(fields.length-1,index));
    const el=fields[active];
    currentSideIndex=active; _wallSideFlash=+el.dataset.i;
    box.querySelectorAll(".aceil-all-row-v13").forEach((r,k)=>r.classList.toggle("is-active",k===active));
    const pos=document.getElementById("aceilAllPositionV13");if(pos)pos.textContent=(active+1)+"/"+fields.length;
    const p=document.getElementById("aceilAllPrevV13"),n=document.getElementById("aceilAllNextV13");
    if(p)p.disabled=active===0;if(n)n.textContent=active===fields.length-1?"✓":"›";
    draw();
    if(selectIt){el.focus({preventScroll:true});try{el.select()}catch(e){}el.closest(".aceil-all-row-v13")?.scrollIntoView({block:"nearest"})}
  };
  fields.forEach((el,index)=>{
    const row=el.closest(".aceil-all-row-v13");
    row.querySelector(".aceil-all-name-v13").onclick=()=>activate(index,true);
    row.querySelector(".aceil-all-arc-v13").onclick=()=>{toggleSideArcType(+el.dataset.i);row.querySelector(".aceil-all-arc-v13").classList.toggle("is-arc",wallTypes[+el.dataset.i]==="arc");activate(index,true)};
    el.onfocus=()=>activate(index,false);
    el.oninput=()=>{const i=+el.dataset.i,v=parseFloat(el.value)||0;lengths[i]=v;row.querySelector(".aceil-all-ok-v13").textContent=v>0?"✓":"";const per=document.getElementById("per");if(per)per.textContent=(_totalPerimeterCm()/100).toFixed(2);draw()};
    el.onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();if(index<fields.length-1)activate(index+1,true);else applyAllSideInputsV11()}};
  });
  document.getElementById("aceilAllPrevV13").onclick=()=>activate(active-1,true);
  document.getElementById("aceilAllNextV13").onclick=()=>active<fields.length-1?activate(active+1,true):applyAllSideInputsV11();
  activate(0,false);
  setTimeout(()=>activate(0,true),120);
}
function closeAllSideInputsV13(){
  _wallSideFlash=-1;document.body.classList.remove("aceil-all-mode-v13");
  closeModal("allSideInputsModalV11");draw();
}
function applyAllSideInputsV11(){
  document.querySelectorAll("#allSideInputsContainerV11 input[data-i]").forEach(el=>{const v=parseFloat(el.value);if(Number.isFinite(v)&&v>0)lengths[+el.dataset.i]=v});
  closeAllSideInputsV13();draw();if(typeof saveState==="function")saveState();
}
new MutationObserver(function(){const anyOpen=document.querySelector(".modal-overlay.open, #elementsModal.open, #rmWallTapMenuV84.open, #rmDirectCurveModalV87.open"),btn=document.getElementById("addRoomBtn");if(btn){const want=anyOpen?"none":"";btn.style.display!==want&&(btn.style.display=want)}}).observe(document.body,{attributes:!0,subtree:!0,attributeFilter:["class","style"]});let currentSideIndex=0;function focusPrevSide(){const arr=[...document.querySelectorAll(".bulk-side-input")];arr.length&&(currentSideIndex=Math.max(0,currentSideIndex-1),arr[currentSideIndex].focus())}function focusNextSide(){const arr=[...document.querySelectorAll(".bulk-side-input")];arr.length&&(currentSideIndex=Math.min(arr.length-1,currentSideIndex+1),arr[currentSideIndex].focus())}function applySideInputs(){[...document.querySelectorAll(".bulk-side-input")].forEach(el=>{const i=+el.dataset.i;lengths[i]=parseFloat(el.value)||0});document.querySelectorAll("input[data-i]").forEach(el=>{const i=+el.dataset.i;lengths[i]&&(el.value=lengths[i],el.classList.add("filled"))});let per=_totalPerimeterCm();document.getElementById("per").textContent=(per/100).toFixed(2),_wallSideFlash=-1,closeModal("sideInputModal"),rebuild(),saveState(),setTimeout(()=>{try{window.A·CEILMeasureConfidence&&typeof window.A·CEILMeasureConfidence.notify==="function"&&window.A·CEILMeasureConfidence.notify(true)}catch(e){window.__diagSilent&&window.__diagSilent(e)}},250)}const cv=document.getElementById("cv"),ctx=cv.getContext("2d");let viewScale=1,viewOffsetX=0,viewOffsetY=0,pinchStartDist=0,pinchStartScale=1,isPanning=!1,panStartX=0,panStartY=0,pts=[],lengths=[],realPts=[],notes=[],closed=!1,diagonalMode=!1,triangleMode=!1,selectedPoint=null,diagonals=[],diagonalOverrides={},circleMode=!1,circleDiamCm=0,lightMarks=[],lightMode=null,wallMarks=[],linearElements=[],wallTypes=[],arcPoints=[],_wallEditId=null,_wallSideFlash=-1;function undoPoint(){closed||circleMode||0===pts.length||(pts.pop(),saveState(),draw())}function N(i){return String.fromCharCode(64+(i+1))}function toggleShapeMenu(){document.getElementById("shapeMenu")?.classList.toggle("open")}function closeShapeMenu(){document.getElementById("shapeMenu")?.classList.remove("open")}function toggleDiagMenu(){document.getElementById("diagMenu")?.classList.toggle("open")}function closeDiagMenu(){document.getElementById("diagMenu")?.classList.remove("open")}function openRectModal(){closed||pts.length>0?showToast('Спочатку очистіть полотно (кнопка "Очистити все")'):(document.getElementById("rectModal").classList.add("open"),setTimeout(()=>document.getElementById("rectW").focus(),100))}function openCircleModal(){closed||pts.length>0?showToast('Спочатку очистіть полотно (кнопка "Очистити все")'):(document.getElementById("circleModal").classList.add("open"),setTimeout(()=>document.getElementById("circleDiam").focus(),100))}function closeModal(id){"sideInputModal"===id&&(_wallSideFlash=-1,draw()),document.getElementById(id).classList.remove("open")}function applyRect(){const w=parseFloat(document.getElementById("rectW").value),h=parseFloat(document.getElementById("rectH").value);if(!w||!h||w<=0||h<=0)return void showToast("Введіть коректні розміри");closeModal("rectModal"),resetAllSilent();const scale=Math.min(580/w,580/h),cw=w*scale,ch=h*scale,ox=85+(580-cw)/2,oy=85+(580-ch)/2;pts=[{x:ox,y:oy},{x:ox+cw,y:oy},{x:ox+cw,y:oy+ch},{x:ox,y:oy+ch}],lengths=[w,h,w,h],realPts=[{x:0,y:0},{x:w,y:0},{x:w,y:h},{x:0,y:h}],closed=!0,autoOpenDimensionsAfterClose(),circleMode=!1;let labels=["AB (ширина)","BC (висота)","CD (ширина)","DA (висота)"],h2="<table><thead><tr><th>Стор.</th><th>см</th></tr></thead><tbody>";for(let i=0;i<4;i++)h2+=`<tr><td style="font-weight:bold;">${labels[i]}</td><td><input type="number" inputmode="decimal" data-i="${i}" value="${lengths[i]}" oninput="checkMark(this); saveLen(this)" onkeydown="nextInput(event,this)" class="filled"></td></tr>`;h2+="</tbody></table>",document.getElementById("tbl").innerHTML=h2;const perimeter=2*(w+h),area=w*h;document.getElementById("per").textContent=(perimeter/100).toFixed(2),document.getElementById("area").textContent=(area/1e4).toFixed(2),document.getElementById("inCorners").textContent="4",document.getElementById("outCorners").textContent="0";const diag=Math.hypot(w,h);document.getElementById("diagList").innerHTML=`<b>AC</b>=${diag.toFixed(0)}см`,updateCornerCount(),draw(),updateChecks(),saveState()}function applyCircle(){const diam=parseFloat(document.getElementById("circleDiam").value);if(!diam||diam<=0)return void showToast("Введіть коректний діаметр");closeModal("circleModal"),resetAllSilent(),circleDiamCm=diam,circleMode=!0,closed=!0,autoOpenDimensionsAfterClose();const r=diam/2,perimeter=Math.PI*diam,area=Math.PI*r*r;document.getElementById("per").textContent=(perimeter/100).toFixed(2),document.getElementById("area").textContent=(area/1e4).toFixed(2),document.getElementById("inCorners").textContent="—",document.getElementById("outCorners").textContent="—",document.getElementById("diagList").innerHTML=`<b>Діаметр</b>: ${diam} см`,document.getElementById("tbl").innerHTML=`\n<table><thead><tr><th>Параметр</th><th>Значення</th></tr></thead><tbody><tr><td>Діаметр</td><td><b>${diam} см</b></td></tr><tr><td>Радіус</td><td><b>${r.toFixed(1)} см</b></td></tr><tr><td>Периметр</td><td><b>${(perimeter/100).toFixed(2)} м</b></td></tr><tr><td>Площа</td><td><b>${(area/1e4).toFixed(2)} м²</b></td></tr></tbody></table>`,draw()}
