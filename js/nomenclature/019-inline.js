...
function openAllSideInputsModalV11(){
  ...
  let h='<div class="aceil-all-dims-v11">';
  for(let i=0;i<pts.length;i++){
    const j=(i+1)%pts.length,isArc="arc"===wallTypes[i],name=N(i)+N(j);
    h+=`<div class="aceil-all-dim-row-v11" data-side-row="${i}">
      <button type="button" class="aceil-all-dim-name-v11">${name}</button>
      <div class="aceil-all-dim-input-v11"><input type="number" inputmode="decimal" class="aceil-all-dim-field-v11" data-i="${i}" value="${lengths[i]||""}" placeholder="0"><span>см</span></div>
      <button type="button" class="aceil-all-dim-arc-v11 ${isArc?"is-arc":""}" data-i="${i}" title="Пряма / дуга">〜</button>
      <span class="aceil-all-dim-ok-v11">${lengths[i]>0?"✓":""}</span>
    </div>`;
  }
  h+='</div>';
  ...
  if(!box)return;
  box.innerHTML=h;
  document.body.classList.add("aceil-all-dims-workspace-v12");
  document.getElementById("allSideInputsModalV11").classList.add("open");
  const fields=[...box.querySelectorAll(".aceil-all-dim-field-v11")];
  const prevBtn=document.getElementById("aceilAllDimsPrevV12"),nextBtn=document.getElementById("aceilAllDimsNextV12");
  const goTo=(index)=>{
    if(!fields.length)return;
    index=Math.max(0,Math.min(fields.length-1,index));
    const el=fields[index];
    el.focus({preventScroll:true});
    try{el.select()}catch(e){}
    el.closest(".aceil-all-dim-row-v11")?.scrollIntoView({block:"nearest",behavior:"smooth"});
  };
  const activate=(el,index)=>{
    currentSideIndex=index;
    _wallSideFlash=+el.dataset.i;
    box.querySelectorAll(".aceil-all-dim-row-v11").forEach(r=>r.classList.toggle("is-active",+r.dataset.sideRow===_wallSideFlash));
    if(prevBtn)prevBtn.disabled=index<=0;
    if(nextBtn){nextBtn.disabled=index>=fields.length-1;nextBtn.textContent=index>=fields.length-1?"✓":"›"}
    draw();
  };
  if(prevBtn)prevBtn.onclick=()=>goTo(currentSideIndex-1);
  if(nextBtn)nextBtn.onclick=()=>{
    if(currentSideIndex<fields.length-1)goTo(currentSideIndex+1);
    else applyAllSideInputsV11();
  };
  fields.forEach((el,index)=>{
    const row=el.closest(".aceil-all-dim-row-v11");
    row.querySelector(".aceil-all-dim-name-v11").onclick=()=>{el.focus();el.select()};
    row.querySelector(".aceil-all-dim-arc-v11").onclick=()=>{
      const i=+el.dataset.i;
      toggleSideArcType(i);
      row.querySelector(".aceil-all-dim-arc-v11").classList.toggle("is-arc",wallTypes[i]==="arc");
      el.focus();
    };
    el.addEventListener("focus",()=>activate(el,index));
    el.addEventListener("input",()=>{
      const i=+el.dataset.i,v=parseFloat(el.value)||0;
      lengths[i]=v;
      row.querySelector(".aceil-all-dim-ok-v11").textContent=v>0?"✓":"";
      const per=document.getElementById("per");if(per)per.textContent=(_totalPerimeterCm()/100).toFixed(2);
      draw();
    });
    el.addEventListener("keydown",e=>{
      if(e.key==="Enter"||e.key==="ArrowDown"){e.preventDefault();if(index<fields.length-1)goTo(index+1);else applyAllSideInputsV11()}
      else if(e.key==="ArrowUp"){e.preventDefault();if(index>0)goTo(index-1)}
    });
  });
  fields.forEach((el,i)=>{el.setAttribute("enterkeyhint",i===fields.length-1?"done":"next")});
  setTimeout(()=>{if(fields[0])goTo(0)},80);
}
function applyAllSideInputsV11(){
  const fields=[...document.querySelectorAll("#allSideInputsContainerV11 .aceil-all-dim-field-v11")];
  fields.forEach(el=>{const i=+el.dataset.i,v=parseFloat(el.value);if(Number.isFinite(v)&&v>0)lengths[i]=v});
  _wallSideFlash=-1;
  document.body.classList.remove("aceil-all-dims-workspace-v12");
  closeModal("allSideInputsModalV11");
  draw();saveState&&saveState();
}
