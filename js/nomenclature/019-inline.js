...
/* v14 split-screen all-dimensions mode.
   The original one-wall editor remains untouched. */
function openAllSideInputsModalV11(){
  ...
  const box=document.getElementById("allSideInputsContainerV14");
  if(!panel||!box)return;

  let h='<div class="aceil-all-table-v14">';
  for(let i=0;i<pts.length;i++){
    const j=(i+1)%pts.length,name=N(i)+N(j),isArc=wallTypes[i]==="arc";
    h+=`<div class="aceil-all-row-v14" data-row="${i}">
      <button type="button" class="aceil-all-name-v14">${name}</button>
      <div class="aceil-all-input-v14">
        <input type="number" inputmode="decimal" enterkeyhint="${i===pts.length-1?"done":"next"}"
          data-i="${i}" value="${lengths[i]||""}" placeholder="0">
        <span>см</span>
      </div>
      <button type="button" class="aceil-all-arc-v14 ${isArc?"is-arc":""}" title="Пряма / дуга">〜</button>
      <span class="aceil-all-ok-v14">${lengths[i]>0?"✓":""}</span>
    </div>`;
  }
  h+='</div>';
  box.innerHTML=h;

  /* Close the old one-wall sheet first, then enter a real page split.
     No overlay is used, so the canvas stays visible and live. */
  closeModal("sideInputModal");
  document.body.classList.add("aceil-all-split-v14");
  panel.hidden=false;

  const fields=[...box.querySelectorAll("input[data-i]")];
  let active=0;

  const activate=(index,focusIt)=>{
    if(!fields.length)return;
    active=Math.max(0,Math.min(fields.length-1,index));
    const el=fields[active];
    currentSideIndex=active;
    _wallSideFlash=+el.dataset.i;
    box.querySelectorAll(".aceil-all-row-v14").forEach((r,k)=>r.classList.toggle("is-active",k===active));
    const counter=document.getElementById("aceilAllCounterV14");
    if(counter)counter.textContent=(active+1)+"/"+fields.length;
    const prev=document.getElementById("aceilAllPrevV14"),next=document.getElementById("aceilAllNextV14");
    if(prev)prev.disabled=active===0;
    if(next)next.textContent=active===fields.length-1?"✓":"›";
    draw();
    if(focusIt){
      try{el.focus({preventScroll:true})}catch(e){el.focus()}
      try{el.select()}catch(e){}
      el.closest(".aceil-all-row-v14")?.scrollIntoView({block:"nearest"});
    }
  };

  fields.forEach((el,index)=>{
    const row=el.closest(".aceil-all-row-v14");
    row.querySelector(".aceil-all-name-v14").onclick=()=>activate(index,true);
    row.querySelector(".aceil-all-arc-v14").onclick=()=>{
      const i=+el.dataset.i;
      toggleSideArcType(i);
      row.querySelector(".aceil-all-arc-v14").classList.toggle("is-arc",wallTypes[i]==="arc");
      activate(index,true);
    };
    el.addEventListener("focus",()=>activate(index,false));
    el.addEventListener("input",()=>{
      const i=+el.dataset.i,v=parseFloat(el.value)||0;
      lengths[i]=v;
      row.querySelector(".aceil-all-ok-v14").textContent=v>0?"✓":"";
      const per=document.getElementById("per");
      if(per)per.textContent=(_totalPerimeterCm()/100).toFixed(2);
      draw();
    });
    el.addEventListener("keydown",e=>{
      if(e.key==="Enter"){
        e.preventDefault();
        if(index<fields.length-1)activate(index+1,true);
        else applyAllSideInputsV14();
      }
    });
  });

  document.getElementById("aceilAllPrevV14").onclick=()=>activate(active-1,true);
...
function applyAllSideInputsV14(){
  document.querySelectorAll("#allSideInputsContainerV14 input[data-i]").forEach(el=>{
    const v=parseFloat(el.value);
    if(Number.isFinite(v)&&v>0)lengths[+el.dataset.i]=v;
  });
  closeAllSideInputsV14(false);
  draw();
