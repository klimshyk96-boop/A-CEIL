...
function currentSingleState(){
  ...
  try{st.closed=!!closed}catch(_){st.closed=false}
  try{st.diagonals=clone(typeof diagonals!=="undefined"?diagonals:[])}catch(_){st.diagonals=[]}
  try{st.diagonalOverrides=clone(typeof diagonalOverrides!=="undefined"?diagonalOverrides:{})}catch(_){st.diagonalOverrides={}}
  try{st.circleMode=!!circleMode}catch(_){st.circleMode=false}
  try{st.circleDiamCm=n(typeof circleDiamCm!=="undefined"?circleDiamCm:0)}catch(_){st.circleDiamCm=0}
  try{st.notes=clone(typeof notes!=="undefined"?notes:[])}catch(_){st.notes=[]}
  ...
  try{st.linearElements=clone(typeof linearElements!=="undefined"?linearElements:[])}catch(_){st.linearElements=[]}
  try{st.wallTypes=clone(typeof wallTypes!=="undefined"?wallTypes:[])}catch(_){st.wallTypes=[]}
  try{st.arcPoints=clone(typeof arcPoints!=="undefined"?arcPoints:[])}catch(_){st.arcPoints=[]}
  return st;
}
function currentMetric(id){var el=document.getElementById(id);return el?String(el.textContent||"").trim():""}
function reportObject(){
  var obj=activeObject();
  if(obj&&Array.isArray(obj.rooms)&&obj.rooms.length)return obj;
  var live=currentSingleState(),saved=obj?parse(obj.state):{},hasSaved=(Array.isArray(saved.pts)&&saved.pts.length>=3)||(saved.circleMode&&n(saved.circleDiamCm)>0)||Array.isArray(saved.elemItems)&&saved.elemItems.length;
  var st=hasSaved?saved:live,hasContent=(Array.isArray(st.pts)&&st.pts.length>=3)||(st.circleMode&&n(st.circleDiamCm)>0)||(Array.isArray(st.elemItems)&&st.elemItems.length)||(Array.isArray(st.lightMarks)&&st.lightMarks.length);
  if(!obj&&!hasContent)return null;
  var area=obj&&obj.area!=null?obj.area:currentMetric("area"),per=obj&&obj.per!=null?obj.per:currentMetric("per"),inC=obj&&obj.inC!=null?obj.inC:currentMetric("inCorners"),outC=obj&&obj.outC!=null?obj.outC:currentMetric("outCorners"),thumb=String(obj&&obj.thumb||"");
  if(!thumb&&!hasSaved)try{if(typeof getCanvasThumb==="function")thumb=getCanvasThumb(.4)||""}catch(_){}
  var base=obj?clone(obj):{};
  base.name=String(base.name||((typeof _currentProjName!=="undefined"&&_currentProjName)||"Поточний проєкт"));
  base.addr=String(base.addr||((typeof _currentProjAddr!=="undefined"&&_currentProjAddr)||""));
  base.phone=String(base.phone||((typeof _currentProjPhone!=="undefined"&&_currentProjPhone)||""));
  base.multiRoom=false;
  base.rooms=[{id:"single",name:String(base.roomName||base.comment||"Кімната 1"),area:area,per:per,inC:inC,outC:outC,thumb:thumb,state:st}];
  return base;
}
function sbClient(){
...
function appReturnUrl(){
  try{
    ...
    u.search="";u.hash="";
    if(/^\/report\/[a-f0-9]{20,64}\/?$/i.test(u.pathname))u.pathname="/";
    return u.href;
...
function planImage(room,st){
  var saved=String(room&&room.thumb||"");
  if(/^data:image\/(?:png|jpe?g|webp);base64,/i.test(saved))return saved;
  var pts=Array.isArray(st&&st.pts)?st.pts.filter(function(p){return p&&isFinite(+p.x)&&isFinite(+p.y)}):[];
  var W=160,H=96,pad=9,svg="";
  if(pts.length>=3){
    var xs=pts.map(function(p){return +p.x}),ys=pts.map(function(p){return +p.y});
    var minX=Math.min.apply(null,xs),maxX=Math.max.apply(null,xs),minY=Math.min.apply(null,ys),maxY=Math.max.apply(null,ys);
    var scale=Math.min((W-2*pad)/Math.max(1,maxX-minX),(H-2*pad)/Math.max(1,maxY-minY));
    var ox=(W-(maxX-minX)*scale)/2-minX*scale,oy=(H-(maxY-minY)*scale)/2-minY*scale;
    function px(x){return Math.round((+x*scale+ox)*10)/10}function py(y){return Math.round((+y*scale+oy)*10)/10}
    svg+='<polygon points="'+pts.map(function(p){return px(p.x)+","+py(p.y)}).join(" ")+'" fill="#f8fbff" stroke="#172554" stroke-width="2.4" stroke-linejoin="round"/>';
...
function roomStats(room){
  ...
  return{name:String(room&&room.name||"Кімната"),plan:planImage(room,st),area:area,per:per,corners:inC+outC,lights:lightCount,total:total,profiles:profiles,canvas:canvas,insert:insert,lighting:lighting,other:other};
}
...
function makeHtml(obj,data){
  ...
  var modeLabel=rooms.length>1?"Багатокімнатний":"Однокімнатний",roomsTitle=rooms.length>1?"Розбивка по кімнатах":"Кімната";
  var fin=[
  ...
  var roomsHtml=rooms.map(function(r,i){
    ...
    return '<div class="md-room"><div class="md-room-num">'+(i+1)+'</div><button class="md-plan-btn" type="button" aria-label="Відкрити план '+esc(r.name)+'" onclick="document.getElementById(\'mdPlanModalImg\').src=this.querySelector(\'img\').src;document.getElementById(\'mdPlanModalTitle\').textContent=this.getAttribute(\'data-room\');document.getElementById(\'mdPlanModal\').classList.add(\'show\')" data-room="'+esc(r.name)+'"><img src="'+esc(r.plan)+'" alt="План '+esc(r.name)+'"></button><div class="md-room-name">'+esc(r.name)+'</div><div><small>Площа</small><b>'+fmt(r.area,2)+' м²</b></div><div><small>Периметр</small><b>'+fmt(r.per,2)+' м</b></div><div><small>Світло</small><b>'+fmt(r.lights,0)+' шт</b></div><div><small>Профілі</small><b>'+fmt(prof,2)+' м</b></div><div class="md-room-sum">'+money(r.total)+'</div></div>';
  }).join("");
  ...
  '<section class="md-object"><div class="md-object-main"><small>Обʼєкт</small><h2>'+esc(obj.name||"—")+'</h2><p>'+esc(obj.addr||"")+'</p><p>'+esc(obj.phone||"")+'</p></div><div><small>Кімнат</small><b>'+rooms.length+'</b></div><div><small>Дата</small><b>'+new Date().toLocaleDateString("uk-UA")+'</b></div><div><small>Режим</small><b>'+modeLabel+'</b></div><div><small>Статус</small><b>Актуальний</b></div></section>'+
  ...
  '<section class="md-rooms"><h3>'+roomsTitle+'</h3>'+roomsHtml+'</section></div><div class="md-plan-modal" id="mdPlanModal" onclick="if(event.target===this)this.classList.remove(\'show\')"><div class="md-plan-card"><div class="md-plan-head"><span id="mdPlanModalTitle">План кімнати</span><button class="md-plan-close" type="button" onclick="document.getElementById(\'mdPlanModal\').classList.remove(\'show\')">×</button></div><img id="mdPlanModalImg" alt="План кімнати"></div></div><script>(function(){var publicToken=new URLSearchParams(location.search).get("r");if(!new RegExp("^/report/[a-f0-9]{20,64}/?$","i").test(location.pathname)&&!new RegExp("^[a-f0-9]{20,64}$","i").test(publicToken||""))return;var appUrl='+JSON.stringify(returnUrl)+',back=document.getElementById("mdBack"),cloud=document.getElementById("mdCloud"),share=document.getElementById("mdShare"),box=document.getElementById("mdCloudBox"),inp=document.getElementById("mdCloudUrl"),copy=document.getElementById("mdCopy");function reportUrl(){return location.origin+"/report/"+(publicToken||location.pathname.split("/").filter(Boolean).pop())}function showUrl(){inp.value=reportUrl();box.classList.add("show");cloud.textContent="✓ Хмарне посилання";return inp.value}back.onclick=function(){location.href=appUrl||"/"};cloud.onclick=showUrl;copy.onclick=async function(){var u=showUrl();try{await navigator.clipboard.writeText(u);copy.textContent="✓ Скопійовано";setTimeout(function(){copy.textContent="Копіювати"},1500)}catch(e){inp.focus();inp.select()}};share.onclick=async function(){var u=reportUrl();try{if(navigator.share)await navigator.share({title:"Менеджерський звіт A·CEIL",url:u});else{await navigator.clipboard.writeText(u);showUrl()}}catch(e){}};})();<\/script></body></html>';
}
window.A_CEIL_PublishManagerReport=async function(){
  var obj=reportObject();
  if(!obj||!Array.isArray(obj.rooms)||!obj.rooms.length)throw new Error("Немає даних для менеджерського звіту");
  ...
  return window.A_CEIL_ReportLinks.publishPayload(payload,{projectId:obj._dbId||obj.id});
};
function bindLocalReport(win,onBack){
  ...
  cloud.addEventListener("click",function(){getCloud().catch(function(){})});
...
function injectManagerCard(){
  ...
  var obj=reportObject(),old=document.getElementById("A_CEIL_ManagerReportCard");
  if(!obj||!Array.isArray(obj.rooms)||!obj.rooms.length){if(old)old.remove();return}
  if(old)return;
  ...
  card.innerHTML='<div class="mr-kicker">'+(obj.rooms.length>1?'Багатокімнатний':'Однокімнатний')+' режим</div><div class="mr-title">📊 Менеджерський звіт</div><div class="mr-sub">Ключові показники, профілі, полотно, вставка, освітлення, фінанси та план одним екраном.</div><button type="button">Відкрити менеджерський звіт</button>';
