...
function makeHtml(obj,data){
  ...
  '<section class="md-rooms"><h3>Розбивка по кімнатах</h3>'+roomsHtml+'</section></div><script>(function(){if(!new RegExp("^/report/[a-f0-9]{20}/?$","i").test(location.pathname))return;var appUrl='+JSON.stringify(returnUrl)+',back=document.getElementById("mdBack"),cloud=document.getElementById("mdCloud"),share=document.getElementById("mdShare"),box=document.getElementById("mdCloudBox"),inp=document.getElementById("mdCloudUrl"),copy=document.getElementById("mdCopy");function showUrl(){inp.value=location.href;box.classList.add("show");cloud.textContent="✓ Хмарне посилання";return inp.value}back.onclick=function(){location.href=appUrl||"/"};cloud.onclick=showUrl;copy.onclick=async function(){var u=showUrl();try{await navigator.clipboard.writeText(u);copy.textContent="✓ Скопійовано";setTimeout(function(){copy.textContent="Копіювати"},1500)}catch(e){inp.focus();inp.select()}};share.onclick=async function(){var u=showUrl();try{if(navigator.share)await navigator.share({title:"Менеджерський звіт A·CEIL",url:u});else await navigator.clipboard.writeText(u)}catch(e){}};})();<\/script></body></html>';
}
