...
function openReportInsideApp(canvas,fileName){
  ...
  layer.innerHTML='<div class="aceil-ir-head"><button type="button" data-close>← Назад</button><strong>Звіт A·CEIL</strong><span></span></div><div class="aceil-ir-body"><div class="aceil-ir-zoom" aria-label="Масштаб звіту"><button type="button" data-zoom-out aria-label="Зменшити">−</button><b data-zoom-label>100%</b><button type="button" data-zoom-in aria-label="Збільшити">＋</button></div><div class="aceil-ir-cloud" hidden><input readonly data-cloud-url><button type="button" data-copy>Копіювати</button><button type="button" data-open>Відкрити</button></div><img alt="Звіт A·CEIL"></div><div class="aceil-ir-actions"><button type="button" data-save>Зберегти PNG</button><button type="button" data-cloud>Хмарне посилання</button><button type="button" class="primary" data-share>Поділитися</button></div>';
  if(!document.getElementById("A_CEIL_InAppImageReportCss")){
    var style=document.createElement("style");style.id="A_CEIL_InAppImageReportCss";style.textContent="#A_CEIL_InAppImageReport{position:fixed;inset:0;z-index:2147483000;background:#eef3f9;color:#0f172a;display:grid;grid-template-rows:auto 1fr auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif}.aceil-ir-head{height:58px;padding:8px 14px;background:#fff;border-bottom:1px solid #dbe4ef;display:grid;grid-template-columns:1fr auto 1fr;align-items:center}.aceil-ir-head button{justify-self:start;border:1px solid #dbe4ef;background:#f8fafc;border-radius:11px;padding:9px 13px;font-weight:800;color:#0f172a}.aceil-ir-body{overflow:auto;padding:16px;overscroll-behavior:contain;-webkit-overflow-scrolling:touch}.aceil-ir-body img{display:block;width:100%;max-width:none;height:auto;margin:0 auto;background:#fff;border-radius:15px;box-shadow:0 8px 28px rgba(15,23,42,.13);transform-origin:top center;touch-action:pan-x pan-y}.aceil-ir-zoom{position:sticky;top:0;z-index:5;width:max-content;margin:0 0 10px auto;padding:4px;display:flex;align-items:center;gap:3px;background:rgba(15,23,42,.90);border-radius:13px;box-shadow:0 7px 22px rgba(15,23,42,.24);backdrop-filter:blur(8px)}.aceil-ir-zoom button{width:38px;height:36px;border:0;border-radius:10px;background:#334155;color:#fff;font-size:22px;font-weight:800;line-height:1}.aceil-ir-zoom button:disabled{opacity:.35}.aceil-ir-zoom b{min-width:52px;text-align:center;color:#fff;font-size:12px}.aceil-ir-cloud{position:sticky;top:52px;z-index:3;max-width:900px;margin:0 auto 12px;padding:9px;background:#fff;border:1px solid #bfdbfe;border-radius:13px;display:flex;gap:8px;box-shadow:0 6px 20px rgba(15,23,42,.12)}.aceil-ir-cloud[hidden]{display:none}.aceil-ir-cloud input{flex:1;min-width:0;border:0;background:#f8fafc;border-radius:9px;padding:10px;color:#334155}.aceil-ir-cloud button{border:0;background:#dbeafe;color:#1d4ed8;border-radius:9px;padding:10px 13px;font-weight:850}.aceil-ir-actions{display:flex;gap:10px;padding:10px 14px max(12px,env(safe-area-inset-bottom));background:#fff;border-top:1px solid #dbe4ef}.aceil-ir-actions button{flex:1;border:1px solid #bfdbfe;background:#eff6ff;color:#1d4ed8;border-radius:13px;padding:13px;font-weight:850}.aceil-ir-actions .primary{background:#2563eb;color:#fff;border-color:#2563eb}";document.head.appendChild(style)
  }
  document.body.appendChild(layer);document.body.style.overflow="hidden";var reportBody=layer.querySelector(".aceil-ir-body"),reportImg=layer.querySelector("img"),zoomOut=layer.querySelector("[data-zoom-out]"),zoomIn=layer.querySelector("[data-zoom-in]"),zoomLabel=layer.querySelector("[data-zoom-label]"),zoom=1;reportImg.src=dataUrl;
  function setZoom(value){zoom=Math.max(.75,Math.min(2.5,Math.round(value*20)/20));reportImg.style.width=Math.round(zoom*100)+"%";zoomLabel.textContent=Math.round(zoom*100)+"%";zoomOut.disabled=zoom<=.75;zoomIn.disabled=zoom>=2.5}
  zoomOut.onclick=function(){setZoom(zoom-.25)};zoomIn.onclick=function(){setZoom(zoom+.25)};zoomLabel.onclick=function(){setZoom(1)};
  var pinchDistance=0,pinchZoom=1;
  reportImg.addEventListener("touchstart",function(e){if(e.touches.length===2){pinchDistance=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);pinchZoom=zoom}},{passive:true});
  reportImg.addEventListener("touchmove",function(e){if(e.touches.length!==2||!pinchDistance)return;e.preventDefault();var d=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);setZoom(pinchZoom*d/pinchDistance)},{passive:false});
  reportImg.addEventListener("touchend",function(e){if(e.touches.length<2)pinchDistance=0},{passive:true});
  setZoom(1);
  ...
  openButton.onclick=function(){if(cloudInput.value)window.open(cloudInput.value,"_blank","noopener")};
