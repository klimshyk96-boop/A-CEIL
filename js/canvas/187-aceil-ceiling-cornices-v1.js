(function(){
  ...
  function localShape(item){
    ...
    if(item.shape==="U")return [[0,0],[sx*a,0],[sx*a,sy*b],[0,sy*b]];
  ...
  function totalLength(item){return item.shape==="straight"?(+item.lengthA||0):item.shape==="U"?(+item.lengthA||0)+2*(+item.lengthB||0):(+item.lengthA||0)+(+item.lengthB||0);}
  ...
  function drawCornices(ctx){
    ...
    list().forEach(function(item){
      ...
      var mid=ps[Math.floor(ps.length/2)],label=(item.shape==="straight"?"Прямий":item.shape==="U"?"П-подібний":"Г-подібний")+" · "+(Math.round(totalLength(item))/100).toFixed(2)+" м";
  ...
  function ensureStyle(){
    ...
    style.textContent='#ceilingCorniceModal{display:none;position:fixed;inset:0;z-index:10050;background:rgba(15,23,42,.55);backdrop-filter:blur(7px);-webkit-backdrop-filter:blur(7px);align-items:flex-end;justify-content:center;padding:10px}#ceilingCorniceModal.open{display:flex}.cc-card{width:min(96vw,480px);max-height:88dvh;overflow:auto;-webkit-overflow-scrolling:touch;background:#fff;border-radius:26px;padding:17px;box-shadow:0 24px 70px rgba(15,23,42,.28)}.cc-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px}.cc-title{font-size:20px;font-weight:950;color:#0f172a}.cc-sub{font-size:11px;font-weight:750;color:#64748b;margin-top:3px}.cc-close{width:38px!important;height:38px!important;min-height:38px!important;padding:0!important;background:#f1f5f9!important;color:#334155!important;box-shadow:none!important}.cc-label{display:block;margin:11px 0 5px;font-size:10.5px;font-weight:950;color:#64748b;text-transform:uppercase;letter-spacing:.06em}.cc-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.cc-input,.cc-select{width:100%;height:44px;padding:9px 11px;border:1.5px solid #e2e8f0;border-radius:14px;background:#fff;color:#0f172a;font-size:14px;font-weight:800;box-sizing:border-box}.cc-choice{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}.cc-choice button,.cc-directions button,.cc-colors button{min-height:42px!important;padding:7px!important;background:#f8fafc!important;color:#334155!important;border:1.5px solid #e2e8f0!important;box-shadow:none!important}.cc-choice button.active,.cc-directions button.active,.cc-colors button.active{background:linear-gradient(135deg,#2563eb,#6366f1)!important;color:#fff!important;border-color:transparent!important}.cc-directions{display:grid;grid-template-columns:repeat(4,1fr);gap:7px}.cc-directions button{font-size:24px!important}.cc-colors{display:grid;grid-template-columns:1fr 1fr;gap:7px}.cc-summary{margin-top:11px;padding:10px 12px;border-radius:14px;background:#eff6ff;color:#1d4ed8;font-size:12px;font-weight:850}.cc-footer{display:grid;grid-template-columns:auto 1fr 1.25fr;gap:8px;margin-top:13px;position:sticky;bottom:-17px;padding:10px 0 17px;background:linear-gradient(180deg,rgba(255,255,255,.7),#fff 30%)}.cc-footer button{min-height:46px!important}.cc-delete{background:#fff1f2!important;color:#be123c!important;border:1px solid #fecdd3!important;box-shadow:none!important}.cc-cancel{background:#f1f5f9!important;color:#475569!important;box-shadow:none!important}.cc-save{background:linear-gradient(135deg,#2563eb,#6366f1)!important;color:#fff!important}.cc-launch-icon{font-size:21px}.cc-profile-note{font-size:10px;color:#94a3b8;font-weight:700;margin-top:4px}@media(max-width:420px){.cc-card{padding:15px}.cc-footer{grid-template-columns:1fr 1fr}.cc-delete{grid-column:1/-1;order:3}}';
    document.head.appendChild(style);
  }
  function ensureModal(){
    if(id("ceilingCorniceModal"))return;
    var modal=document.createElement("div");modal.id="ceilingCorniceModal";
    modal.innerHTML='<div class="cc-card"><div class="cc-head"><div><div class="cc-title" id="ccTitle">Карниз на стелі</div><div class="cc-sub">Окремо від стін · точне розташування</div></div><button type="button" class="cc-close" onclick="closeCeilingCorniceModal()">×</button></div><div class="cc-label">Форма</div><div class="cc-choice" id="ccShapes"><button type="button" data-value="straight">— Прямий</button><button type="button" data-value="L">Г-подібний</button><button type="button" data-value="U">П-подібний</button></div><div class="cc-grid"><div><label class="cc-label">Сторона A, см</label><input class="cc-input" id="ccLengthA" type="number" inputmode="decimal" value="70"></div><div><label class="cc-label" id="ccLengthBLabel">Сторона B, см</label><input class="cc-input" id="ccLengthB" type="number" inputmode="decimal" value="140"></div></div><div class="cc-label">Напрям кута</div><div class="cc-directions" id="ccDirections"><button type="button" data-x="0" data-y="0">┌</button><button type="button" data-x="1" data-y="0">┐</button><button type="button" data-x="0" data-y="1">└</button><button type="button" data-x="1" data-y="1">┘</button></div><div class="cc-label">Колір</div><div class="cc-colors" id="ccColors"><button type="button" data-value="white">⚪ Білий</button><button type="button" data-value="black">⚫ Чорний</button></div><div class="cc-grid"><div><label class="cc-label">Тип</label><select class="cc-select" id="ccMounting"><option value="hidden">Прихований</option><option value="surface">Накладний</option><option value="niche">Ніша</option></select></div><div><label class="cc-label">Кількість рядів</label><select class="cc-select" id="ccRows"><option value="1">1 ряд</option><option value="2">2 ряди</option><option value="3">3 ряди</option></select></div></div><div class="cc-label">Від якого кута кімнати</div><select class="cc-select" id="ccBase"></select><div class="cc-grid"><div><label class="cc-label">Відступ X, см</label><input class="cc-input" id="ccOffsetX" type="number" inputmode="decimal" value="50"></div><div><label class="cc-label">Відступ Y, см</label><input class="cc-input" id="ccOffsetY" type="number" inputmode="decimal" value="50"></div></div><label class="cc-label">Профіль номенклатури</label><input class="cc-input" id="ccProfile" placeholder="Можна прив’язати пізніше"><div class="cc-profile-note">Без вибраного профілю карниз показується і зберігається, але його ціна не додається.</div><div class="cc-summary" id="ccSummary"></div><div class="cc-footer"><button type="button" class="cc-delete" id="ccDelete" onclick="deleteCeilingCornice()">🗑</button><button type="button" class="cc-cancel" onclick="closeCeilingCorniceModal()">Скасувати</button><button type="button" class="cc-save" onclick="saveCeilingCornice()">✓ Зберегти</button></div></div>';
  ...
  function updateSummary(){
    var shape=id("ccShapes")&&id("ccShapes").dataset.value||"L",a=Math.max(0,+id("ccLengthA").value||0),b=Math.max(0,+id("ccLengthB").value||0),total=shape==="straight"?a:shape==="U"?a+2*b:a+b;
    id("ccLengthB").disabled=shape==="straight";id("ccLengthB").style.opacity=shape==="straight"?".45":"1";id("ccDirections").style.display=shape==="straight"?"none":"grid";
    id("ccSummary").textContent="Загальна довжина: "+(total/100).toFixed(2)+" м · "+(shape==="straight"?"прямий":shape==="U"?"П-подібний":"Г-подібний")+" · "+(id("ccRows").value||1)+" ряд";
  }
  function openEditor(item){
    ensureStyle();ensureModal();editingId=item&&item.id||null;
    var value=item||{shape:"L",lengthA:70,lengthB:140,flipX:false,flipY:false,color:"white",mountingType:"hidden",rows:1,baseIndex:0,offsetX:50,offsetY:50,profileName:""};
    ...
    id("ccLengthA").value=Math.round(+value.lengthA||70);id("ccLengthB").value=Math.round(+value.lengthB||140);id("ccMounting").value=value.mountingType||"hidden";id("ccRows").value=String(value.rows||1);fillBaseOptions(value.baseIndex||0);id("ccOffsetX").value=Math.round(+value.offsetX||0);id("ccOffsetY").value=Math.round(+value.offsetY||0);id("ccProfile").value=value.profileName||"";id("ccDelete").style.display=editingId?"block":"none";updateSummary();id("ceilingCorniceModal").classList.add("open");
  }
  ...
  window.saveCeilingCornice=function(){
    ...
    if(!item){item={id:"cornice_"+Date.now()+"_"+Math.floor(Math.random()*1000)};list().push(item);}
    item.shape=shape;item.lengthA=a;item.lengthB=b;item.flipX=id("ccDirections").dataset.x==="1";item.flipY=id("ccDirections").dataset.y==="1";item.color=id("ccColors").dataset.value||"white";item.mountingType=id("ccMounting").value||"hidden";item.rows=Math.max(1,+id("ccRows").value||1);item.baseIndex=Math.max(0,+id("ccBase").value||0);item.offsetX=Math.max(0,+id("ccOffsetX").value||0);item.offsetY=Math.max(0,+id("ccOffsetY").value||0);item.profileName=String(id("ccProfile").value||"").trim();item.totalLengthM=Math.round(totalLength(item))/100;
    persistNow();try{if(typeof window.saveState==="function")window.saveState();}catch(e){}redraw();window.closeCeilingCorniceModal();toast("✓ Карниз збережено: "+item.totalLengthM.toFixed(2)+" м");
  };
